...
public class BrowsePresenter extends BasePresenter<BrowseView> implements SectionPresenter, VideoGroupPresenter, AccountChangeListener {
    ...
    public void onViewInitialized() {
        ...
        if (mCounterRefreshRunnable == null) {
            mCounterRefreshRunnable = new Runnable() {
                @Override
                public void run() {
                    Log.d("VideoCounterDebug", "20-minute auto-refresh triggered.");
                    fetchNewVideoCounts();
    ...
    private void fetchNewVideoCounts() {
        Log.d("VideoCounterDebug", "fetchNewVideoCounts() started");
        if (getContext() == null) {
            Log.d("VideoCounterDebug", "getContext() is null, aborting");
            return;
        }
        
        boolean isEnabled = getMainUIData().isNewVideoCounterEnabled();
        Log.d("VideoCounterDebug", "isNewVideoCounterEnabled=" + isEnabled);
        ...
        Log.d("VideoCounterDebug", "Iterating mSections. Size=" + mSections.size());
        for (BrowseSection section : mSections) {
            ...
            if (isPinned && isVideo) {
                ...
                Log.d("VideoCounterDebug", "Found Pinned Video Section! id=" + section.getId() + " title=" + section.getTitle() + " hasChannel=" + hasChan + " chanId=" + item.channelId);
                ...
                if (hasChan) {
                    ...
                    Log.d("VideoCounterDebug", "hasChannel() was false but channelId exists: " + item.channelId);
                    mPinnedChannelIds.put(section.getId(), item.channelId);
                    mOriginalTitles.put(section.getId(), cleanTitle);
                }
            } else if (isPinned) {
                 Log.d("VideoCounterDebug", "Section is pinned but data is not Video. Data class: " + (section.getData() != null ? section.getData().getClass().getName() : "null"));
            }
        }

        Log.d("VideoCounterDebug", "mPinnedChannelIds size: " + mPinnedChannelIds.size());
        ...
        for (Map.Entry<Integer, String> entry : mPinnedChannelIds.entrySet()) {
            ...
            Disposable action = getContentService().getRssFeedObserve(channelId)
                    ...
                    .subscribe(
                        mediaGroup -> {
                            Log.d("VideoCounterDebug", "onNext received for channel " + channelId);
                            if (mediaGroup == null || getView() == null) {
                                Log.d("VideoCounterDebug", "mediaGroup or view null. mediaGroup=" + (mediaGroup==null) + " view=" + (getView()==null));
                                return;
                            }

                            List<MediaItem> items = mediaGroup.getMediaItems();
                            if (items == null || items.isEmpty()) {
                                Log.d("VideoCounterDebug", "mediaGroup items null or empty for channel " + channelId);
                                return;
                            }
                            ...
                            Log.d("VideoCounterDebug", "Channel: " + channelId + " newestFound: " + newestFetchedData + " lastKnown: " + lastKnownData);

                            if (lastKnownData == null) {
                                Log.d("VideoCounterDebug", "First time fetching for " + channelId + ". Storing baseline.");
                                android.widget.Toast.makeText(getContext(), "First fetch for " + channelId + ", baseline saved.", android.widget.Toast.LENGTH_SHORT).show();
                                ...
                                if (!found) {
                                    ...
                                    Log.d("VideoCounterDebug", "Fallback hit. new count by date=" + newVideoCount);
                                ...
                                Log.d("VideoCounterDebug", "Final new video count for " + channelId + " is " + newVideoCount);
                                if (newVideoCount > 0) {
                                    android.widget.Toast.makeText(getContext(), "New videos for " + channelId + ": " + newVideoCount, android.widget.Toast.LENGTH_SHORT).show();
                                }
                        ...
                        error -> {
                            Log.e("VideoCounterDebug", "onError fetching RSS for counter " + channelId, error);
                        }
                );
    ...
    private void resetNewVideoCounter(int sectionId) {
        ...
        Log.d("VideoCounterDebug", "Resetting counter for section " + sectionId + " with data: " + currentNewestData);
        android.widget.Toast.makeText(getContext(), "Resetting counter for section " + sectionId, android.widget.Toast.LENGTH_SHORT).show();
